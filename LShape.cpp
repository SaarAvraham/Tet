...
lShape::lShape(ShapeDir shapeDir, char material)
{
...
bool lShape::move(ARROWS key, GameBoard & gameBoard, Menu & menu)
{
...
Shape::ShapeDir lShape::getShapeDir() const
{
	return shapeDir;
}

void lShape::rotateRight(GameBoard & gameBoard, Menu &menu)
{
	Point lastPos[LSHAPE_LEN];
	Point pCheck[LSHAPE_LEN];
	for (size_t i = 0; i < LSHAPE_LEN; i++)
		lastPos[i] = body[i];
	for (size_t i = 0; i < LSHAPE_LEN; i++)
		pCheck[i] = body[i];
...
void lShape::immidiateDown(GameBoard & gameBoard, Menu & menu)
{
	bool resume = true;
...
void lShape::updateBoard(char keyPressed, GameBoard & gameBoard, Menu& menu, Point lastPos[Shape::MAXLEN]) const
{
	int i;
	if (lastPos != NULL)
	{
...
bool lShape::hasReachedBottom(GameBoard &gameBoard, Menu& menu) 
{
